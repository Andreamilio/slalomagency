...
const talents = [
  { name:'Frankie',      handle:'@frankie',  ig:'2.4M', tt:'1.1M', img:'/roster/frankie.avif', mt:0,   ar:'3/4.2' },
  { name:'Giulia',       handle:'@giulia',   ig:'1.8M', tt:'3.2M', img:'/roster/giulia.avif',  mt:44,  ar:'3/4'   },
  { name:'Sofi',         handle:'@sofi',     ig:'900K', tt:'2.1M', img:'/roster/sofi.avif',    mt:-20, ar:'3/4.4' },
  { name:'Gionny',       handle:'@gionny',   ig:'3.1M', tt:'4.8M', img:'/roster/gionny.avif',  mt:22,  ar:'3/4'   },
]

export default function Roster() {
  return (
    <section id="roster" className="roster-section" style={{ background:'var(--surface)', padding:'130px 24px', position:'relative', overflow:'hidden', zIndex:2 }}>
      <div style={{ content:'', position:'absolute', inset:0, pointerEvents:'none', backgroundImage:'repeating-linear-gradient(-45deg,transparent,transparent 60px,rgba(200,255,0,.016) 60px,rgba(200,255,0,.016) 61px)' }} />
      <div style={{ maxWidth:1200, margin:'0 auto' }}>
        ...
        <div className="reveal" style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:52, flexWrap:'wrap', gap:20, position:'relative', zIndex:1 }}>
          ...
          <a href="#" className="link-arr" style={{ display:'inline-flex', alignItems:'center', gap:8, fontFamily:'Figtree, sans-serif', fontWeight:700, fontSize:12.5, color:'var(--lime)', textDecoration:'none', letterSpacing:'.05em', textTransform:'uppercase' }}>
            Vedi tutti →
          </a>
        </div>
        ...
        <div className="reveal roster-grid" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16, alignItems:'start', position:'relative', zIndex:1 }}>
          {talents.map((t) => (
            <div key={t.handle} className="r-card roster-card" style={{ aspectRatio:t.ar, marginTop:t.mt }}>
              {/* Real photo */}
              <div className="r-bg" style={{ position:'absolute', inset:0, zIndex:0 }}>
                <Image
                  src={t.img}
                  alt={t.name}
                  fill
              ...
              <div style={{ position:'absolute', bottom:0, left:0, right:0, padding:18, zIndex:4 }}>
                ...
                <div style={{ display:'flex', gap:6, marginTop:10, flexWrap:'wrap' }}>
                  <a href={`https://instagram.com/${t.handle.slice(1)}`} target="_blank" rel="noopener" style={{ display:'inline-flex', alignItems:'center', gap:5, fontSize:10, fontWeight:800, letterSpacing:'.04em', color:'#080810', padding:'4px 10px', borderRadius:8, background:'var(--lime)', textDecoration:'none' }}>
                    <IgIcon />{t.ig}
                  </a>
                  <a href={`https://tiktok.com/${t.handle}`} target="_blank" rel="noopener" style={{ display:'inline-flex', alignItems:'center', gap:5, fontSize:10, fontWeight:800, letterSpacing:'.04em', color:'#080810', padding:'4px 10px', borderRadius:8, background:'rgba(240,240,248,.9)', textDecoration:'none' }}>
                    <TtIcon />{t.tt}
                  </a>
                </div>
              </div>
